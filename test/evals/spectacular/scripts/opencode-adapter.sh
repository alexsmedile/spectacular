...
eval_attempt=0
until "$eval_opencode" run \
    --dir "$eval_workspace" \
    --auto \
    --model "$eval_model" \
    --format json \
    -- \
    "$(cat "$eval_raw.prompt")" > "$eval_raw"; do
  eval_attempt=$((eval_attempt + 1))
  if [ "$eval_attempt" -ge 2 ]; then
    echo "opencode run failed after retry" >&2
    exit 1
  fi
  echo "opencode run failed; retrying once after 30s" >&2
  sleep 30
done
