...
| `soft-db-substrate` | review (M1-M6 done) | v1.5.0 | Memory + sessions soft-folder DBs (MEMORY.md/memory/ + SESSIONS.md/sessions/); CLI mutators `decide`, `remember`, `session start\|end`; auto-link to active session; doctor `memory` + `sessions` areas with 4h stale warning; coding-kit triggers updated. All version manifests at 1.5.0, doctor clean. Awaiting commit + tag. |
| `snapshot-tidy` | planned | v1.6.0 | Move root-level `*@v*.md` snapshots into `.spectacular/snapshots/<DOC>/v<x.y>.md`; add `doctor snapshots` area + `--fix` migration. Pure substrate hygiene; independent of memory-protocols. |
| `feedback-loop` | review (M0-M6 done) | v1.6.0 | New skill mode for prototyping-stage human-feedback acquisition. 5-step loop, `.spectacular/feedback/` + per-request `feedback/` substrate, `doctor feedback` area (30-day stale warning), hidden aliases (`iterate`/`experiment`/`test`/`probe`/`try`). PRINCIPLES.md §9 codifies feedback ≠ verification ≠ benchmark. Self-dogfooded; 4 CLI fixes applied (list cols, slug truncation, `--promote-hint`, require `--next-action`). All manifests at 1.6.0, CHANGELOG + README updated, doctor clean. Awaiting user `/release`. |
| `memory-protocols` | planned (M0 grill pending) | v1.6.0 | Memory protocols layer on top of v1.5.x. 6 user-prioritized usecase families (session/checkpoint recall, large-codebase surfing, PRINCIPLES persistence, handoffs, large-repo onboarding, SPEC-drift detection) + 8 candidate patterns from research + 6 new user-suggested ideas (load-manifest MEMORY.md, onboard verb, pinned docs, drift watchdog, checkpoints, cross-repo memory). 12 open questions block milestone breakdown. Gated on soft-db-substrate dogfooding ≥1 week. |
