...
| **`ideas/`** | Pre-commitment workbench | A speculative "might build this" being clarified — it may carry questions, research links, alternatives, and a non-binding working plan, but has no execution commitment | `ideas/index.md` index + `<slug>.md` entries | `spectacular idea new` → `promote` | [[idea-rules]] |
...
| **Prototype artifact** | A request/vision/feedback-owned mock used for human validation | It inherits its owner's context; `PRT` remains reserved rather than creating a parallel lifecycle. |
