...
If moving to review, also evaluate the 2-of-6 rule (see [[plan-rules]] § 2-of-6; canonical: [[verify-authoring]]) — only propose creating VERIFY.md when the request actually warrants one. Default for doc-only / refactor / spec requests is **no VERIFY.md** — use PLAN § Validation or add a `### Verification` group to TASKS.md instead.
...
Per [[verify-authoring]], "opt-in" refers to **whether a standalone VERIFY.md file gets scaffolded** — not whether verification runs. The 2-of-6 rule decides the file; verification itself is mandatory.
