...
| **`ideas/`** | Pre-commitment spark | Speculative "might build this" — no lifecycle, no scope, may never matter again | `ideas/index.md` index + `<slug>.md` entries | `spectacular idea new` → `promote` | [[idea-rules]] |
| **`questions/`** | Active ambiguity | A blocker or choice that must remain visible until answered, commonly requiring human product/business judgment | `questions/QUE-NNN-<slug>.md` entries | `spectacular question new\|resolve` | [[question-rules]] |
