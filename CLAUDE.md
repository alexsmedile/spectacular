...
| `ideas-doctype` | review (M1-M5 done) | v1.7.0 | Registered `ideas` as first-class doc-type — `idea-rules.md`, template, doc-index row, CLI verbs (`idea new\|list\|promote`), doctor area (90-day stale warn on `exploring`), SKILL triggers. Status enum: `parked\|exploring\|promoted`; promotion target `archive/ideas/`. Mirrors `feedback` shape. All 5 manifests at 1.7.0, doctor clean (0/0/10), dogfood passed. Awaiting commit + tag. |
| `read-verbs` | review (M1-M5 done) | v1.8.0 | Added 11 read-only CLI verbs: `requests\|decisions\|memories\|sessions` (list), `request\|decision\|memory <slug>` (detail), `sessions show <slug>`, `show <doctype>`, `summary`, `progress <slug>`, `paths`. Universal `--status\|--since\|--limit\|--all\|--json`. Skim-by-default, `--full` opt-in. SKILL.md routing audited — all 28 CLI verbs documented including the previously-missing `migrate` row. All 5 manifests at 1.8.0, doctor clean (0/0/10). Awaiting commit + tag. |
