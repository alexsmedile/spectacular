...
func TestYAMLGraphsAreRefused(t *testing.T) {
	t.Parallel()
	...
	tests := map[string]string{
		...
		"cyclic alias": prefix +
			...
			"  self: *loop\n" +
			"---\n",
		"reversed key order": prefix +
			"zeta: &shared\n" +
			"  value: retained\n" +
			"alpha: *shared\n" +
			"---\n",
		"cross-property alias": prefix +
			"alpha:\n" +
			"  item: &shared [one, two]\n" +
			"zeta:\n" +
			"  item: *shared\n" +
	...
	want := "refused unsupported_yaml_graph: " + unsupportedYAMLGraphDetail
	for name, input := range tests {
		input := input
		t.Run(name, func(t *testing.T) {
			t.Parallel()
			_, err := Parse([]byte(input))
			if !domain.RefusalHasCode(err, domain.RefusalUnsupportedYAMLGraph) {
				t.Fatalf("Parse error = %v, want unsupported_yaml_graph", err)
			}
			if err.Error() != want {
