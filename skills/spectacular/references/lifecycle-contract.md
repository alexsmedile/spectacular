...
| Question | `open → deferred | resolved` | Deferral needs a reason; resolution records answer provenance | QUE entry | Resolved questions remain durable |
...
| Specification | `draft | unconfirmed → approved → implemented → superseded | deprecated → archived` | See specification gates below | SPC entry | Archived file moves to `archive/specs/` |
...
`PRT` and `TSK` are reserved identifiers only. They have no standalone collection or lifecycle until a concrete consumer is designed.
...
- `archived` means removed or irrelevant; the complete record leaves active context but remains explicitly retrievable.
...
Requests own `source_spec`. Spec-to-request backlinks are derived so one SPC can safely produce several requests. Replacement specs own `supersedes`; `superseded_by` is derived.
...
No legacy label becomes `implemented` without a user-supplied commit/build reference. Canonical folders are plural collections: `ideas/`, `questions/`, `research/`, `spikes/`, `decisions/`, `specs/`, `memories/`, `sessions/`, `feedbacks/`, `audits/`, `fixes/`, and `debugs/`.
