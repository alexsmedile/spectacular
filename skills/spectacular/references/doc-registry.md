...
<doc-id>:                              # required, kebab-case, used in triggers
  ...
  mode: grill | structured | append | freeform | reference  # required
...
- `structured` — walk slots in order **per repeating section** (ROADMAP-style: same 6 slots filled for each version block). Sections can be added, edited, or skipped independently. Review gate runs per-section.
- `append` — append a single new entry to the existing file (e.g. ADR entries in DECISIONS.md); no slot loop
- `freeform` — scaffold the template and exit; no interactive slot-fill (used for docs where structure is loose: AGENTS, STACK)
- `reference` — contract/spec doc not generated by users; loaded on demand by the engine (e.g. `migrations-contract`, `kits-contract`)
...
roadmap:
  ...
  snapshot-on-edit: true
  overrides: references/roadmap-overrides.md
  description: "Per-version scope + phase + exit criteria (structured grill; 9-phase chain)"
