...
func (s Service) ProposalView(ref string) (map[string]any, error) {
	...
	// This is a retrieval card, not a preparation gate.  Early, valid Proposals
	// deliberately need only the common record grammar, so show must never
	// invent a candidate or turn absent optional planning fields into a refusal.
	metadata := map[string]any{}
	if proposal.Document.Record.CreatedBy != nil {
		metadata["created_by"] = *proposal.Document.Record.CreatedBy
	}
	if proposal.Document.Record.Created != nil {
		metadata["created"] = *proposal.Document.Record.Created
	}
	if proposal.Document.Record.Updated != nil {
		metadata["updated"] = *proposal.Document.Record.Updated
	}
	value := map[string]any{
		"noun": "Proposal", "ref": refOf(proposal), "id": proposal.Document.Record.ID.String(),
		"title": value(proposal.Document.Record.Title), "status": value(proposal.Document.Record.Status),
		"description": value(proposal.Document.Record.Description), "path": proposal.Path, "fingerprint": proposal.Fingerprint,
		"metadata": metadata, "candidate_authoritative": false,
		"omissions": []string{},
	}
	for _, field := range []string{"target_contract", "base_version", "base_fingerprint"} {
		if text := mustString(proposal.Document, field); text != "" {
			value[field] = text
		} else {
			value["omissions"] = append(value["omissions"].([]string), field+" absent")
		}
	}
	value["delta"] = map[string]any{"additions": mustStrings(proposal.Document, "additions"), "modification_from": mustStrings(proposal.Document, "modification_from"), "modification_to": mustStrings(proposal.Document, "modification_to"), "removals": mustStrings(proposal.Document, "removals")}
	if newCapability, boolErr := workspace.Bool(proposal.Document, "new_capability", false); boolErr == nil {
		value["new_capability"] = newCapability
		if candidate, candidateErr := s.candidate(proposal); candidateErr == nil {
			value["candidate_contract"] = candidate
		} else {
			value["omissions"] = append(value["omissions"].([]string), "candidate_contract unavailable")
		}
	} else {
		value["omissions"] = append(value["omissions"].([]string), "new_capability absent")
		value["omissions"] = append(value["omissions"].([]string), "candidate_contract absent")
	}
	return value, nil
}
