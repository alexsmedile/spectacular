...
| Root and singular-path cleanup is mixed into schema design | Scope creep and misleading migration necessity | The generated singular `debug/` trace was removed; former `.DS_Store`, `.last-mutation`, and `migrations.log` claims are no longer live on main | Route any future cleanup separately; only schema-relevant rules enter a newly allocated hardening SPC |
