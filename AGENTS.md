...
When changing canonical Spectacular docs or skill behavior, consult `.spectacular/` for current project intent before editing — start with `.spectacular/AGENTS.md` (operating rules), `.spectacular/PRD.md` (intent), and `.spectacular/specs/index.md` (what's built). Keep `.spectacular.local/` personal and uncommitted. Do not overwrite versioned skill snapshots in `skills/spectacular/versions/`; add a new snapshot when intentionally releasing a new skill version. When working on a request inside `.spectacular/requests/<slug>/`, load that folder's `PLAN.md` and `TASKS.md`, plus any `specs/<capability>/SPEC.md` it references — not the whole `specs/` tree.
