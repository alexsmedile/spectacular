...
doc-id: spec
mode: stub
location: .spectacular/specs/index.md
scope: project-wide
...
**Doc-specific role — the index.** SPEC.md is the **index** of system capabilities: dense bullets pointing to per-capability spec files under `specs/<capability>.md` when needed (see request `spec-refactor`).
...
- **Index check** — any Capabilities bullet spanning >2 lines or containing sub-bullets fails with: "promote to `specs/<capability>.md` and leave a one-line pointer." SPEC.md stays an index; prose accretion is drift.
