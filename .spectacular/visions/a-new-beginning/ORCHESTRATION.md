...
| Boundary | Use | Persistence and authority |
|---|---|---|
| Current control/Mission session | Default for tightly coupled or sequential work and anything using already-loaded context | Keeps end-to-end ownership; may plan, execute, repair, prove, and reconcile within its envelope |
| Subagent | Closed “do it” brief, no owner dialogue, checkable output, disposable context; optionally parallel only when files/work are disjoint | Returns findings/artifacts and disappears; owns no lifecycle or durable status |
| Reused persistent thread | Same objective, authority, and canonical artifact need further owner dialogue or later resume | Continue by delta from its last accepted checkpoint |
| New persistent thread | The conversation itself needs a distinct durable purpose, owner interaction, or long-lived domain boundary | Requires a stable job title, typed handoff, return destination, and declared reason it cannot stay in the existing thread |
| Fresh reviewer | Consequence requires independent assurance | Read-only, exact commit/tree, final findings; not a new implementation owner |
| Cold verifier | Recovery without prior chat is the claim | Receives only allowed cold-start inputs and returns recovery evidence |

Normally keep one persistent control thread and at most one persistent active-Mission thread.
Decision threads are exceptional: use one only for a coherent multi-turn Type-1 owner interview
that would contaminate or outlive the active Mission. Preparation, implementation, repair,
re-review, and return are stages inside the Mission thread, not reasons to create more threads.

### Conversation-locality and delta handoffs

A persistent thread is a semantic cache. Reuse it while its objective, authority, and owned
artifact remain stable. Continuations carry only:

```yaml
continue_from: <accepted checkpoint or exact commit/tree>
new_evidence: [<new pointers only>]
scope_delta: <none or exact change>
authority_delta: <none or exact change>
requested_result: <one bounded outcome>
```

Do not replay the constitutional stack, rebuild a complete status summary, or create a relay thread
when durable pointers already answer those questions. A new thread must declare one earned reason:
durable owner conversation, independent evidence, cold-context proof, parallel disjoint work, or
context-window protection with a designed join. Phase names, role names, branches, and repair
rounds are not earned reasons.
...
H26 completed M1 preparation with explicit owner dispositions and final charter approval. Central
