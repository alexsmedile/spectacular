...
- `review` → **structure check** (not a placeholder check): every `### <id>` block has a `check`; `severity` is `block | warn` or absent; every `## @<hook>` is one of the locked 9; no orphan sections; `principle:` (if present) is a positive integer. Report violations; the mechanical subset also runs in `doctor policies`.
