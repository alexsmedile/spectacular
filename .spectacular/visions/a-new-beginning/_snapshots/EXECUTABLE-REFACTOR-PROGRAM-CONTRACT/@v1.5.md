...
supersedes: _snapshots/EXECUTABLE-REFACTOR-PROGRAM-CONTRACT/@v1.4.md
upstream:
...
This v1.5 program records the owner-authorized M1 activation, the accepted W0 scaffold, and the
decision to stop investing in v1. Earlier program versions are preserved in the snapshot tree; this
unversioned file is the sole current program authority. M1 alone is active. It authorizes no
migration, release, provider effect, or later implementation Mission.
...
W0 shared-scaffold gate — accepted
  → M1 semantic records + canonical workspace substrate — active
  → M2 durable governed Mission loop
  → M3 guided Skill + registry-generated CLI + retrieval/integrity
  → M4 clean-v2 release readiness
```

The program is strictly serialized. M1 is the only active implementation Mission; its H27 return
and independent review are the only next program work. M2 remains blocked. Real project migrations
and cutovers are later project-specific Missions and do not block clean-v2 release readiness.
